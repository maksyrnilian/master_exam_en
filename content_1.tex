...
\section{Conservation laws in physics and their connection with symmetry laws}
    ...
    \subsection{Examples of the application of Noether's theorem}
        ...
        \subsubsection{Conservation of momentum}
            ...
            \end{equation*}
            \todo{Example of how a Langrangian can be *depend* on position.}
...
\label{sec:postulates}
    ...
    \subsection{The photon hypothesis and its consequences}
        ...
        \begin{itemize}
            ...
            \item \textbf{Systems with several degrees of freedom.} If a system has more than one degree of freedom (a photon, for instance, carries both position and polarisation), each degree of freedom has its own state space, and the states of the whole system are built by pairing every state of one degree of freedom with every state of the other. This combined space is called the \emph{tensor product} of the individual spaces.
            \todo[backgroundcolor=green!25,linecolor=green!55!black]{Simplified: described the tensor product in words and dropped the $\otimes$ notation.}
            \todo{Delete the section}
    ...
    \subsection{The wave function}
        ...
        \end{equation*}
        \todo[backgroundcolor=green!25,linecolor=green!55!black]{Checked: the postulate statements, the normalisation condition and the position-basis expansion above are consistent.}
...
\section{Variational principles of physics; examples of equations that can be derived from them}
    ...
    a single equation for the action $S$ (here called Hamilton's principal function). The reason $\Ham$ takes these particular arguments is that each momentum has been replaced by a derivative of the action, $p_i = \partial S/\partial q_i$, so the usual $\Ham(\vec q, \vec p, t)$ is evaluated at $\vec p = \partial S/\partial \vec q$. What matters for us is that its form is strikingly close to the Schrödinger equation, which is why it is regarded as the classical doorway to wave mechanics. The fully quantum version of the whole scheme is \textbf{Feynman's path integral}: rather than selecting one path, it gives \textit{every} path a contribution and adds them together, and when quantum effects are negligible the sum is dominated by the single stationary-action path, recovering the classical principle.
    ...
    \todo[backgroundcolor=green!25,linecolor=green!55!black]{Verified: the brachistochrone (cycloid), Euler--Lagrange $\to$ Newton, Legendre $\to$ Hamiltonian, Hamilton--Jacobi, the path integral, and the electromagnetic/Einstein--Hilbert examples all check out.}
...
\label{sec:mechanical-media}
    ...
    \todo[backgroundcolor=green!25,linecolor=green!55!black]{Checked: ``fluid'' is used throughout for liquids and gases together (as defined above), and the bulk-versus-shear distinction is correct.}
...
\section{Flow of an incompressible liquid; Bernoulli's principle}
    ...
    \end{itemize}
    \todo[backgroundcolor=green!25,linecolor=green!55!black]{Checked: irrotationality is kept because it lets \cref{eq:bernoulli} hold with a single constant throughout the flow, not merely along one streamline.}
    ...
    \todo[backgroundcolor=green!25,linecolor=green!55!black]{Verified: this is the Venturi effect, a direct consequence of \cref{eq:bernoulli}.}
