...
A variational principle expresses the equations of motion as a condition on an entire trajectory rather than on the instantaneous state. Its central object is the \textbf{action} $S$, a \textit{functional}: a rule assigning a single real number to each candidate trajectory. The \textbf{principle of least action} states that the path a system actually follows is the one for which $S$ is \textbf{stationary} --- unchanged to first order under any small deformation of the path (a minimum in the simplest cases, whence the name).

The idea originates in optics, with Fermat's principle that light takes the path of least travel time between two points. Its migration into mechanics grew from the brachistochrone problem (Johann Bernoulli, 1696): the curve down which a bead slides under gravity from one point to another in the \textit{least time} --- a cycloid, notably not the straight line, which only minimises \textit{length}. Extremising such quantities systematically required the calculus of variations, developed by Euler and Lagrange.

For a mechanical system the action is the time integral of the \textbf{Lagrangian} $\Lagr = T - V$ (kinetic minus potential energy), as given in \cref{eq:action}. Requiring the first variation to vanish, $\delta S = 0$, singles out the stationary paths and yields the \textbf{Euler--Lagrange equation} (\cref{eq:euler-lagr}). This reproduces Newton's second law, but written in arbitrary \textit{generalised coordinates} $q_i$ --- any convenient parameters fixing the configuration --- which is what makes the formalism so powerful for constrained or awkward systems.

A Legendre transformation trades the velocities $\dot q_i$ for momenta and leads to the Hamiltonian formulation, built on $\Ham = T + V$, the total energy, which is the natural starting point for statistical physics and quantum mechanics. Carrying the variational idea further and regarding the action as a function of its endpoint gives the Hamilton--Jacobi equation,

\begin{equation}\label{eq:hamilton-jacobi}
\Ham \left( \vec{q}, \frac{\partial S}{\partial \vec{q}}, t \right) + \frac{\partial S}{\partial t} = 0,
\end{equation}

which is strikingly close in form to the Schrödinger equation and is the natural classical gateway to wave mechanics (here $S$ is Hamilton's principal function, equal in value to the action). The quantum counterpart, Feynman's path integral, assigns every trajectory a probability amplitude and recovers the classical, stationary-action path in the appropriate limit.

The principle reaches well beyond mechanics. Applied to the electromagnetic field Lagrangian $\Lagr = -\frac{1}{4}F_{\mu\nu}F^{\mu\nu}$ it produces Maxwell's equations in tensor form, and applied to the Einstein--Hilbert action it produces the full Einstein field equations of general relativity.
