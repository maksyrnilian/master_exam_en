...
Precession is the slow circular drift of the axis of a spinning object. Instead of pointing in a fixed direction, the spin axis itself sweeps around, tracing out the surface of a cone. The familiar example is a spinning top: as it spins, its axis does not simply fall over under gravity but slowly circles around the vertical.

This happens when a torque (a twisting force) acts at right angles to the angular momentum of the spinning body. For the top, gravity supplies this torque.
...
where $\tau$ is the torque, $I_s$ the moment of inertia (a measure of how hard it is to change the body's spin), $\omega_s$ the spin angular velocity (how fast the body spins), and $\theta$ the angle between the spin axis and the vertical axis about which it precesses.

\subsection{Quantum precession}

A particle such as an electron carries a magnetic moment $\vec{\mu}$ associated with its spin. Placed in an external magnetic field $\vec{B}$, this moment feels a torque

\begin{equation}
    \vec{\tau} = \vec{\mu}\times \vec{B},
\end{equation}

which makes it precess around the direction of the field, in direct analogy with gravity making a top precess. This is Larmor precession. Its frequency depends only on the strength of the field and not on the orientation of the spin, which is why it underlies magnetic resonance techniques.

A separate contribution is Thomas precession, which is purely relativistic. It arises because a spin that is accelerating undergoes an additional precession in special relativity, and it enters as a correction to the spin--orbit interaction.
