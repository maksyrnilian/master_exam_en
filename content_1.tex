...
\label{sec:inertial-frames}
    ...
    \begin{enumerate}
        \item Time dilation, described by the formula $\Delta t = \gamma\Delta t_0$, where $\Delta t_0$ is the duration of a phenomenon recorded by an observer at rest with respect to that phenomenon, and $\Delta t$ is the duration of the same phenomenon recorded by another observer moving with velocity $v$ with respect to the first. The event appears \textit{shortest} in its own frame.
        \item Lorentz contraction, described by the formula $L = {L_0}/{\gamma}$, where $L_0$ is the length of an object at rest and $L$ is the length of the object recorded by an observer moving with respect to it at velocity $v$. The object appears \textit{longest} in its own frame.
...
\section{Fundamental interactions; carriers and range of interactions, charges and characteristic energy scales}
    ...
    \todo[backgroundcolor=green!25,linecolor=green!55!black]{Filled in: added the paragraph on energy units, mass--energy equivalence and the $\hbar c$ conversion. Figures computed from the SI constants: $\hbar c = 197.3$~MeV~fm, $1$~eV$/c^{2} = 1.783\times10^{-36}$~kg.}
    Energies here are quoted in \emph{electronvolts}: $1$~eV is the energy an electron gains on being accelerated through a potential difference of one volt, $1\,\mathrm{eV} = 1.602\times10^{-19}$~J, with the usual multiples keV, MeV and GeV. Masses are quoted in the same units, which is possible because of \emph{mass--energy equivalence}, $E = mc^{2}$: a mass is given by the rest energy it corresponds to. Strictly this makes the unit of mass the eV$/c^{2}$, but the $c^{2}$ is almost always left out and one simply says that the $W$ boson has a mass of $80$~GeV. In SI units $1$~eV$/c^{2} = 1.783\times10^{-36}$~kg. One combination is worth remembering, $\hbar c \approx 197$~MeV~fm, because dividing it by a rest energy converts a mass straight into a length, which is exactly what is needed to turn the masses of the carriers into the ranges listed above.

    \todo[backgroundcolor=green!25,linecolor=green!55!black]{Rewritten: the source's argument was muddled and its last sentence had it backwards. Replaced with the standard Yukawa range estimate $R\sim\hbar/mc$, stated as the heuristic it is, with the point you raised made explicit --- the relation concerns the energy spread of a short-lived state, not our knowledge of a fixed mass. Also added the caveat that the strong force's $1$~fm range comes from confinement and pion exchange, not from a gluon mass, which the table alone would contradict.}
...
\section{Conservation laws in physics and their connection with symmetry laws}
    ...
    \subsection{Noether's theorem}
        ...
        \todo[backgroundcolor=green!25,linecolor=green!55!black]{Resolved: kept the definition here (its first occurrence) and gave $S$ something to do --- it is what the equation of motion itself follows from, and a symmetry of $\Lagr$ is a symmetry of $S$, which is why the examples below test $\Lagr$ and not the integral. Added a label to the variational-principles section so it can be cross-referenced from here.}
        The theorem is stated in terms of the action because the action is also what the equation of motion follows from: requiring $S$ to be stationary yields the Euler--Lagrange equation below, which is the principle of least action (\cref{sec:variational}). Since $S$ is nothing but the Lagrangian added up over time, a transformation that leaves $\Lagr$ unchanged leaves $S$ unchanged as well, and it is in fact enough that $\Lagr$ change by a total time derivative. This is why the examples that follow test the symmetry on the Lagrangian rather than on the integral.
    ...
    \subsection{Examples of the application of Noether's theorem}
        ...
        \subsubsection{Conservation of momentum}
            ...
            \todo[backgroundcolor=green!25,linecolor=green!55!black]{Filled in: added the uniform-gravity example of a position-dependent Lagrangian, which also shows that the conservation law holds only in the directions in which the symmetry actually holds.}
...
\label{sec:postulates}
    ...
    \subsection{The photon hypothesis and its consequences}
        ...
        \end{itemize}
        \todo[backgroundcolor=green!25,linecolor=green!55!black]{Deleted as you asked: the bullet on systems with several degrees of freedom and the tensor product. Nothing later in the note depends on it.}
        \todo[backgroundcolor=green!25,linecolor=green!55!black]{Answered: $\delta_{kl}$ is the Kronecker delta, not the Dirac delta --- defined in place in the bullet above, with a note on where the Dirac delta does play that role.}

    \subsection{The law of composition of probability amplitudes}
    \todo[backgroundcolor=green!25,linecolor=green!55!black]{Rewritten: separated the two rules (multiply along a path, add across indistinguishable alternatives) and worked the double slit through explicitly, so that it is clear where the interference term comes from and at which point probabilities rather than amplitudes are added.}
    ...
    \subsection{Measurement of a quantum system}
        ...
        \begin{itemize}
            ...
            \end{equation*}
        \end{itemize}
        \todo[backgroundcolor=green!25,linecolor=green!55!black]{Answered: the notation clash flagged in the source. $\ket{\psi}$ is the state of the system and the $\ket{\phi_{k}}$ are the outcome states picked out by the apparatus, now said explicitly in the bullet above.}
    ...
    \subsection{The wave function}
        ...
        \todo[backgroundcolor=green!25,linecolor=green!55!black]{Answered: what $\ket{x}$ means and why the expansion is an integral rather than a sum.}
...
\label{sec:variational}
    ...
    \todo[backgroundcolor=green!25,linecolor=green!55!black]{Resolved: rather than repeating the display, the action is now cross-referenced to its definition (\cref{eq:action}) at its first mention here, and the Noether section says why it is introduced there. Say if you would rather the display itself moved down here and Noether referred forwards to it.}
