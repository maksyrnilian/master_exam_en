...
\label{sec:postulates}
    ...
    \subsection{The photon hypothesis and its consequences}
        ...
        \begin{itemize}
            ...
            \item \textbf{Systems with several degrees of freedom.} If a system has more than one degree of freedom (a photon, for instance, carries both position and polarisation), each degree of freedom has its own Hilbert space $H_i$, and the space of the whole system is their tensor product, $H = \otimes_i H_i$.
            \todo{Delete or simplify above, what is a tensor product and what is this weird symbol?}
    ...
    \subsection{The wave function}
        ...
        \end{equation*} \todo{QM section needs to be checked}
...
\section{The laws of thermodynamics}
    ...
    \subsection{The second law of thermodynamics}
        ...
        Spontaneous processes in an isolated system drive it towards higher entropy, and equilibrium is the state of maximum entropy. Microscopically, entropy counts the number of microstates $\Omega$ compatible with the macrostate, $S = \kB\log\Omega$ (\cref{eq:boltzmann-entropy}): left to itself a system drifts towards the macrostate realised by the overwhelmingly largest number of microstates, which is why entropy tends to increase.
...
\section{Flow of an incompressible liquid; Bernoulli's principle}
    ...
    \begin{itemize}
        ...
        \item the flow of the liquid is steady and irrotational: at any fixed point in the pipe the velocity, density and so on do not change with time, and the fluid elements are carried along without spinning about themselves \todo{is irrotational needed?}
    \end{itemize}
    ...
    where we have, in turn, the kinetic energy, the potential energy and the energy due to pressure of a unit of fluid. For a chosen piece of liquid this equation will always be satisfied, and it permits the description of flow through a pipe changing its cross-section or height. It also explains the so-called hydrodynamic paradox. Where a pipe narrows, incompressibility forces the liquid to flow faster (the same volume must pass each second through a smaller area); by \cref{eq:bernoulli} the larger $v^2/2$ must then be offset by a smaller pressure $p$. The counter-intuitive result is that the pressure is \emph{lower} where the pipe is narrow and the flow is fast. \todo{verify if true}
