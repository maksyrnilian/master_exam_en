...
\begin{equation}\label{eq:maxwell-vacuum}
    ...
    \end{alignedat}
\end{equation}
where $\vec{J}$ is the current density, $\vec{B}$ is the magnetic induction field. \todo{free vs bound distinction? reuse notes from plazmonika?}
...
We define the electric displacement field $\vec{D}$ and the magnetic field $\vec{H}$ as:

\begin{equation}\label{eq:constitutive-DH}
    \begin{aligned}
...
where $\vec{P}$ and $\vec{M}$ are the medium polarisation (\cref{sec:electric-field}) and magnetisation (\cref{sec:magentic-field}) respectively.
...
\end{aligned}
\end{equation} \todo{same as above}
