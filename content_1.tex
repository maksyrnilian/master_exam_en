...
\label{sec:postulates}
    ...
    \subsection{Language of quantum mechanics}\label{sec:qm-language}
    ...
    \todo{Add a matric multiplication rule}

    \subsection{Postulates of quantum mechanics}
        
        Historically the quantisiation of energy, $E = \hbar\omega$, would be considered a defining feature of quantum mechanics or a postulate, but the reality is more nuanced. An unbound system has continuous energy spectrum, and the quantisation of energy is a consequence of the boundary conditions or confinement of the system. It is a mathematical feature similar to a guitar string having qunatised frequencies. The postulates of quantum mechanics (QM) are fundamental.

        \paragraph{Quantum State}

            The state of a quantum system is described by a state vector $\ket{\psi}$, which is an element of a complex Hilbert space $H$. The state vector contains all the information about the system, and its evolution is governed by the Schrödinger equation. The state vector can be expressed in different bases, such as position or momentum, and can be represented as a wave function $\psi(x)$ in the position basis.

        \paragraph{Operators}

            All measurable quantities (observables) are represented by Hermitian lienar operators $\hat{A}$ acting on the Hilbert space of states. The eigenvalues of the operator are the possible results of a measurement of the observable, and the corresponding eigenvectors are the states in which the system is found after the measurement.

        \paragraph{Observables and the collapse}

            The only values that are obtained in a measurement of an 
            observable “A” are the eigenvalues “an” of the corresponding operator “ A ˆ”. The 
            measurement changes the state of the system to the eigenfunction of A ˆ with 
            eigenvalue an. 

        \paragraph{Born rule of probability}

            the average value of an observable corresponding to A ˆ is...

        \paragraph{Evolution}

            The time evolution of the state vector is governed by the Schrödinger equation, which describes how the state changes over time. The Schrödinger equation is a linear partial differential equation that relates the time derivative of the state vector to the Hamiltonian operator, which represents the total energy of the system.
