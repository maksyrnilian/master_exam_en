...
\label{sec:thermal-radiation}
    ...
    \subsection{The black body and Planck's law}
        ...
        \end{empheq}
        \keyeqentry{Planck's law of black-body radiation}
        ...
        \begin{empheq}[box=\fbox]{equation}\label{eq:wien-displacement}
            \lambda_{\max} = \frac{b}{T}, \qquad b \simeq \qty{2.9e-3}{\meter\kelvin},
        \end{empheq}
        \keyeqentry{Wien's displacement law}

        and the \emph{Stefan--Boltzmann law} gives the total power radiated per unit area by a black body,

        \begin{empheq}[box=\fbox]{equation}\label{eq:stefan-boltzmann}
            E_{\text{tot}} = \sigma T^{4}, \qquad \sigma \simeq \qty{5.7e-8}{\watt\per\meter\squared\per\kelvin\tothe{4}}.
        \end{empheq}
        \keyeqentry{Stefan--Boltzmann law}
...
\label{sec:ensembles}
    ...
    \subsection{The microcanonical ensemble ($N$, $V$, $E$)}
        ...
        \begin{empheq}[box=\fbox]{equation}\label{eq:boltzmann-entropy}
            S = \kB \ln\Omega .
        \end{empheq}
        \keyeqentry{Boltzmann entropy}
    ...
    \subsection{The thermodynamic potentials}\label{sec:thermo-potentials}
        ...
        The internal energy $U$ is the natural quantity when the system is isolated, and the first law (\cref{eq:first-law}) written for a reversible change reads $dU = T\,dS - p\,dV + \mu\,dN$.\todo{MIGRATION (deferred): \cref{eq:first-law} in \texttt{content\_1.tex} states the first law without the $\mu\,dN$ term, so it is introduced here without warning. MODERATE EDIT---introduce that term in \cref{eq:first-law} or explain why it should not be there} Its natural variables are therefore $S$, $V$ and $N$. \emph{Natural variables} are the ones in which the potential's differential closes up with nothing left over, so that every other quantity follows by differentiating it---here $T = \left(\nicefrac{\partial U}{\partial S}\right)_{V,N}$ and $-p = \left(\nicefrac{\partial U}{\partial V}\right)_{S,N}$. That particular set is inconvenient, because nobody in a laboratory holds entropy fixed---what is easy to hold fixed is temperature, or pressure. Each potential below is $U$ with one such awkward variable traded for the quantity conjugate to it, by subtracting the product of the pair.
    ...
    \subsection{Quantum statistics}\label{subsec:quantum-statistics}
        ...
        \begin{enumerate}
            ...
            \begin{empheq}[box=\fbox]{equation}\label{eq:maxwell-boltzmann-occ}
                \ev{n_\varepsilon} = e^{-\beta(\varepsilon - \mu)}.
            \end{empheq}
            \keyeqentry{Maxwell--Boltzmann occupation number}
            ...
            \begin{empheq}[box=\fbox]{equation}\label{eq:fermi-dirac}
                \ev{n_\varepsilon} = \frac{1}{e^{\beta(\varepsilon - \mu)} + 1}.
            \end{empheq}
            \keyeqentry{Fermi--Dirac distribution}
            \item \textbf{Bose--Einstein}: for bosons, which are indistinguishable but not exclusion-bound:
            \begin{empheq}[box=\fbox]{equation}\label{eq:bose-einstein}
                \ev{n_\varepsilon} = \frac{1}{e^{\beta(\varepsilon - \mu)} - 1}.
            \end{empheq}
...
\label{sec:macroscopic}
    ...
    \subsection{The equation of state}
        ...
        \begin{empheq}[box=\fbox]{equation}\label{eq:ideal-gas}
            pV = nRT ,
        \end{empheq}
        \keyeqentry{Ideal-gas law}

        \begin{empheq}[box=\fbox]{equation}\label{eq:van-der-waals}
            \left(p + \frac{a n^{2}}{V^{2}}\right)\left(V - nb\right) = nRT .
        \end{empheq}
        \keyeqentry{van der Waals equation}
    ...
    \subsection{Magnetic susceptibility}
        ...
        \begin{empheq}[box=\fbox]{equation}\label{eq:magnetic-susceptibilities}
            \chi_{T} = \left(\frac{\partial M}{\partial H}\right)_{T},\quad  \chi_{S} = \left(\frac{\partial M}{\partial H}\right)_{S} .
        \end{empheq}
        \keyeqentry{Isothermal and adiabatic magnetic susceptibilities}
...
\label{sec:relativistic-eqns}\todoper{KG eqn is mentioned in QM1.  Look it up}
    ...
    \begin{empheq}[box=\fbox]{equation}\label{eq:klein-gordon}
        \left( \Box + \frac{m^2c^2}{\hbar^2} \right)\psi(\vec{r},t) = 0,
    \end{empheq}
    \keyeqentry{Klein--Gordon equation}
    ...
    \begin{empheq}[box=\fbox]{equation}\label{eq:dirac}
        (i\hbar \gamma^{\mu}\partial_{\mu} - mc)\Psi(x^{\nu}) = 0 .
    \end{empheq}
    \keyeqentry{Dirac equation}
...
\label{sec:nuclei}
    ...
    \subsection{Binding energy and mass}
        ...
        \begin{empheq}[box=\fbox]{equation}\label{eq:mass-defect}
            \Delta m = Zm_{p} + Nm_{n} - M(Z,N),
        \end{empheq}
        \keyeqentry{Nuclear mass defect}
...
\label{sec:nucleosynthesis}\todoans{Yes, it is the umbrella term: any process that builds atomic nuclei out of pre-existing nucleons or lighter nuclei, which is precisely the four routes listed below. What it does \emph{not} cover is the making of the nucleons themselves out of quarks in the first microsecond---that goes under baryogenesis and hadronisation, and is a stage earlier than anything called nucleosynthesis.}
    ...
    \subsection{The proton--proton chain (the Sun)}
        ...
        \begin{empheq}[box=\fbox]{equation}\label{eq:pp-chain}
            \begin{aligned}
            ...
            \end{aligned}
        \end{empheq}
        \keyeqentry{Proton--proton chain}
...
\label{sec:particles}
    ...
    \subsection{The structure of hadrons}
        ...
        A hadron must be \emph{colour-neutral} overall, and there are exactly two ways of arranging that. Three quarks, one of each colour, make a \emph{baryon}, the proton ($uud$) and the neutron ($udd$) being the two that matter. A quark together with an antiquark of the matching anticolour makes a \emph{meson}, the lightest being the pion. Baryons are therefore composite fermions and mesons composite bosons, which is the two cuts above crossing each other.
